...
namespace DuiLib {
	...
	// 返回值为 Win32 的 RECT，单位为像素整数
	RECT ComputeCoverSourceRect(
		const Gdiplus::Size& imageSize,  // 图A原始尺寸
		const Gdiplus::Size& boxSize     // 区域B尺寸
	) {
	...
	void CRenderEngine::GdiplusDrawGradient(HDC hDC, const RECT& rc, DWORD dwFirst, DWORD dwSecond, bool bVertical, int radius)
	{
		...
		LinearGradientBrush brush(
			gdiplus_rect,
			Color(dwFirst),  // 左侧颜色（红色）
			Color(dwSecond),  // 右侧颜色（蓝色）
			bVertical ? LinearGradientModeVertical: LinearGradientModeHorizontal // 从左到右渐变
		);
		// 填充圆角矩形
		graphics.FillPath(&brush, &path);
