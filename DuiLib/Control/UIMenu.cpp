...
LRESULT CMenuWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    auto menu_ui = GetMenuUI();
    if (menu_ui) {
        menu_ui->m_pWindow = this;
    }
...
{
    auto menu = dynamic_cast<CMenuUI*>(m_pm.GetRoot());
    return menu;
}
...
void CMenuWnd::ResizeMenu()
{
    ...
    MoveWindow(m_hWnd, rc.left, rc.top, rc.GetWidth(), rc.GetHeight(), FALSE);
        ...
        // ������Menu��ǩ��Ϊxml�ĸ��ڵ�
    CContainerUI* pMenuRoot = dynamic_cast<CContainerUI*>(pRoot);
    if (pMenuRoot) {
...
bool CMenuElementUI::DoPaint(HDC hDC, const RECT& rcPaint, CControlUI* pStopControl)
{
    ...
    if (m_bDrawLine) {
        ...
        CMenuElementUI::DrawItemBk(hDC, m_rcItem, m_topRadius, m_bottomRadius);
    ...
    if (event.Type == UIEVENT_BUTTONUP) {
        if (IsEnabled()) {
            ...
            if (hasSubMenu) {
                ...
                CMenuUI* menuUI = dynamic_cast<CMenuUI*>(GetManager()->GetRoot());
                if (menuUI && menuUI->m_pWindow) {
...
void CMenuElementUI::SetTopRadius(int radius)
{
    m_topRadius = radius;
}

int CMenuElementUI::GetTopRadius()
{
    return m_topRadius;
}

void CMenuElementUI::SetBottomRadius(int radius)
{
    m_bottomRadius = radius;
}

int CMenuElementUI::GetBottomRadius()
{
...
void CMenuElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    } else if (_tcsicmp(pstrName, _T("topradius")) == 0) {
        SetTopRadius(_ttoi(pstrValue));
    } else if (_tcsicmp(pstrName, _T("bottomradius")) == 0) {
        SetBottomRadius(_ttoi(pstrValue));
    } else
